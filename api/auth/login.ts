...
import { verifyPassword, signChallenge } from "../_lib/auth.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!requireRateLimit(req, res, "auth-login", 10, 60_000)) return;
  try {
    ...
    return res.status(200).json({ challenge: await signChallenge({ userId: user.id, purpose: "login" }, "30m") });
  } catch (error) {
